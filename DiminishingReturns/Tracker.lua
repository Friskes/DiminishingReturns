...
do
	...
	ICONS = {
		...
		ctrlstun = 408, -- Kidney Shot
		cyclone = 33786, -- Cyclone
		disarm = 676, -- Disarm
		disorient = 118, -- Polymorph
	...
	elseif pClass == "PRIEST" then
		ICONS.silence = 15487 -- Безмолвие
		ICONS.fear = 10890 -- Ментальный крик
		ICONS.horror = 64044 -- Глубинный ужас
	elseif pClass == "PALADIN" then
		ICONS.disorient = 20066 -- Покаяние
		ICONS.fear = 10326 -- Изгнание зла
		ICONS.ctrlstun = 10308 -- Молот правосудия
	end
end
addon.ICONS = ICONS
...
local function ParseCLEU(self, _, timestamp, event, _, srcName, srcFlags, guid, name, flags, spellId, spell)	
	...
	if event == 'UNIT_DIED' then
		...
		if unit ~= nil then -- игнорим nil
			for i = 1, 40 do
				local _, _, _, _, _, _, _, _, _, _, spellID = UnitAura(unit, i)
				if spellID == 5384 then return end
			end
		end
...
timerFrame:SetScript('OnUpdate', function(self, elapsed)
	...
	--local watched = self:GetDatabase() -- не рабочий метод
